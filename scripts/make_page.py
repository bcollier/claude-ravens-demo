...
def build():
    ...
    # ---------------------------------------------------------- agent C payload
    payload = os.path.join(ROOT, "docs", "llm_payload.png")
    if os.path.exists(payload):
        w('<section><div class="sec-head"><span class="lab">Agent C</span>'
          "<h2>What the language model is actually sent</h2></div>")
        w("<p>Both views, in one request: the assembled sheet <em>and</em> every panel "
          "on its own. For a 3&times;3 problem that is <strong>37 content blocks &mdash; "
          "20 pieces of text and 17 images</strong>. The sheet carries the structure "
          "(which panels form a row, which cell is missing); a single cell inside it is "
          "far too small to compare shapes against. The individual panels carry the "
          "detail but lose every spatial relationship. Neither alone is enough.</p>")
        w('<figure class="dia" style="padding:0;border:1px solid var(--rule)">'
          f'<img src="{data_uri(payload)}" alt="The full contents of one API call: a '
          'system prompt, a layout description, the assembled problem sheet, then each '
          'of the eight matrix cells and eight answer options as separate labelled '
          'images, and a JSON schema constraining the reply." '
          'style="width:100%;display:block;background:#fff">'
          "</figure>")
        w('<p class="lab" style="margin-top:14px">Generated by '
          "scripts/make_payload_figure.py from the real request, so it cannot drift "
          "from the code</p>")
        w("<p style=\"margin-top:22px\">Note the token column on the right. Every model "
          "receives the identical seventeen images; what each charges for them varies by "
          "nearly seven times. Compare models on cost, never on tokens.</p>")
        w("</section>")
    ...
    if os.path.exists(epi_path) or os.path.exists(rn_path):
        ...
        if rn:
            w('<div class="cell stack"><span class="cell-tag">1</span>'
              ...
              "<p style=\"margin:6px 0 0\">A relation network &mdash; a CNN plus a "
              "shared MLP over every <em>pair</em> of panels &mdash; trained only on "
              "synthetic matrices, so the real 96 stay a genuine held-out test. It lands "
              "at or below chance.</p>"
              "<p style=\"margin:0;font-size:15px;color:var(--ink-2)\">Two diagnostics "
              "explain why. Blanking every context panel drops it to chance, so it does "
              "read the matrix. But a bug in my generator left 18.6% of training "
              "problems with eight identical panels, whose answer was a copy of the "
              "context &mdash; and the network learned to prefer options that duplicate "
              "something already on the page. It does that on 29% of real problems, "
              "where that option is right only 10% of the time. A heuristic "
              "anti-correlated with truth is how a model gets below chance.</p>"
              "<p style=\"margin:0;font-size:15px;color:var(--s2)\">The symbolic agent "
              "learned the opposite sign on the same signal from real data: "
              "<code>dup_max</code> carries its largest negative weight.</p>"
              "</div>")
        if sp.get("linear"):
            d = sp["linear"]
